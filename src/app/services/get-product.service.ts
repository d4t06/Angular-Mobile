...
export class GetProductService {
   ...
   getProducts(
      ...
      if (state.price) {
         getProductParams['price[]'] = [state.price.from + '', state.price.to + ''];
      }
      ...
      const URL = this.routerParams['searchKey']
         ? `${PRO_URL}/search?q=${this.routerParams['searchKey']}`
         : PRO_URL;

      return this.http
         .get<{ data: ProductResponse }>(URL, { params: getProductParams })
